...
# --- FUNGSI API SESUAI DOKUMEN SERVER 2 ---
def get_otpnum_v2(endpoint, params):
    try:
        # Base URL sesuai teks: https://otpnum.com/api/v2/
        # Kita pastikan endpoint dipanggil dengan "/" di akhir sebelum query params
...
def tampilkan_halaman():
    ...
    # ==========================================================================
    # TAB 1: SMS LOKAL (60 MENIT TERAKHIR - GAYA MODERN)
    # ==========================================================================
    with tab_lokal:
    ...
    # ==========================================================================
    # TAB 2: SEWA NOMOR ONLINE (SERVER 2 - SESUAI TEKS DOKUMEN)
    # ==========================================================================
    with tab_online:
        # 1. CEK SALDO (/balance)
        ...
        with st.container(border=True):
            country_id = 6 # Indonesia sesuai teks
            ...
            if "list_services_v2" not in st.session_state:
                ...
                if res_serv and res_serv.get('success'):
                    # Berdasarkan teks: data -> services -> [list]
            ...
            if st.button("🚀 BELI NOMOR SEKARANG", use_container_width=True, type="primary"):
                if service_id:
                    with st.spinner("Memesan nomor..."):
                        # Endpoint: /order (Sesuai dokumen nomor 5)
                        ...
                        if res_order and res_order.get('success'):
                            ...
                            st.success(f"Dapat nomor: {res_order['data']['number']}")
                            st.rerun()
                        else:
        ...
        if "active_order" in st.session_state:
            ...
            with st.container(border=True):
                ...
                if c_cek.button("🔄 CEK SMS", use_container_width=True):
                    # Endpoint: /status (Sesuai dokumen nomor 6)
                ...
                if c_can.button("❌ CANCEL / SELESAI", use_container_width=True):
                    # Endpoint: /cancel (Sesuai dokumen nomor 7)

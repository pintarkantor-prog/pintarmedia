...
def tampilkan_kendali_tim():
    ...
    try:
        ...
        # 4. FILTER PERIODE (Kolom 'Tanggal')
        if not df_kas.empty:
            df_kas['TGL_TEMP'] = pd.to_datetime(df_kas['Tanggal'], errors='coerce')
            df_k_f = df_kas[(df_kas['TGL_TEMP'].dt.month == bulan_dipilih) & (df_kas['TGL_TEMP'].dt.year == tahun_dipilih)].copy()
        else:
            df_k_f = pd.DataFrame()
        ...
        # ======================================================================
        # --- UI: FINANCIAL COMMAND CENTER (Pake Metrik Lo!) ---
        # ======================================================================
        with st.expander("💰 ANALISIS KEUANGAN & KAS", expanded=True):
            ...
            warna_delta = "normal" if saldo_riil >= 0 else "inverse" # Ini kunci warna lo!
...
# ======================================================================
        # --- 6. RINCIAN GAJI & SLIP (VERSI MEWAH DIAN - SLIM VERSION) ---
        # ======================================================================
        with st.expander("💰 RINCIAN GAJI & SLIP", expanded=False):
            try:
                ...
                for idx, s in df_staff_raw_slip.reset_index().iterrows():
                    ...
                    # --- 1. DATA FINANSIAL (GAPOK & TUNJANGAN) ---
                    ...
                    # --- 2. FILTER BONUS CAIR (DARI KAS) ---
                    bonus_cair_real = 0
                    if not df_k_f.empty:
                        # Cari di Kas bulan ini yang Kategori 'Gaji Tim' dan ada nama stafnya di keterangan
                        mask_bonus = (df_k_f['Kategori'] == 'Gaji Tim') & (df_k_f['Keterangan'].str.upper().str.contains(n_up, na=False))
                        bonus_cair_real = int(df_k_f[mask_bonus]['NOMINAL_CLEAN'].sum())

                    # --- 3. RUMUS FINAL (SLIM) ---
                    v_total_terima = v_gapok + v_tunjangan + bonus_cair_real
                    ada_kerja = True

                    # --- 4. TAMPILAN VCARD (STYLE DIAN) ---
                    with kol_v[idx % 2]:
                        with st.container(border=True):
                            ...
                            <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 10px;">
                                ...
                                <div>
                                    ...
                                    <span style="font-size: 11px; color: #888;">{s.get('LEVEL', 'STAFF')}</span>
                                </div>
                            </div>
                            ...
                            if st.button(f"📄 PREVIEW & PRINT SLIP {n_up}", key=f"vcard_{n_up}", use_container_width=True):
                                ...
                                <div id="slip-gaji-full" style="background: white; padding: 30px; border-radius: 20px; border: 1px solid #eee; font-family: sans-serif; width: 350px; margin: auto; color: #333; box-shadow: 0 10px 30px rgba(0,0,0,0.05);">
                                    <center>
                                        <h2 style="color: #1d976c; margin-bottom: 0;">PINTAR MEDIA</h2>
                                    ...
                                    <div style="background: #fcfcfc; padding: 15px; border-radius: 12px; border: 1px solid #f0f0f0; margin: 20px 0;">
                                        <table style="width: 100%; font-size: 11px; border-collapse: collapse;">
                                            ...
                                            <tr><td style="color: #999; font-weight: 600; text-transform: uppercase;">Level</td><td align="right"><b>{s.get('LEVEL', 'STAFF')}</b></td></tr>
                                    ...
                                    <table style="width: 100%; font-size: 13px; line-height: 2.5; border-collapse: collapse;">
                                        ...
                                        <tr style="color: #1d976c; font-weight: 600;"><td>Bonus Cair (Kas)</td><td align="right">+ {bonus_cair_real:,}</td></tr>
                                    </table>
                                    ...
                                    <div style="margin-top: 30px; text-align: center; font-size: 9px; color: #bbb; border-top: 1px solid #f5f5f5; padding-top: 15px;">
                                        <b>Diterbitkan secara digital oleh Sistem PINTAR MEDIA v2.0</b><br>
                                        Waktu Cetak: {sekarang.strftime('%d/%m/%Y %H:%M:%S')} WIB
                                    </div>
                                </div>
                                <div style="text-align: center; margin-top: 20px;">
                                    <button onclick="window.print()" style="padding: 12px 25px; background: #1a1a1a; color: #55efc4; border: 2px solid #55efc4; border-radius: 10px; font-weight: bold; cursor: pointer;">🖨️ PRINT SLIP</button>
                                </div>
                                """
                ...
                st.error(f"⚠️ Gagal memuat Slip Gaji: {e}")

...
def tampilkan_kendali_tim():
    ...
    pilihan_nama = c_bln.selectbox("📅 Pilih Bulan:", list(daftar_bulan.values()), index=sekarang.month - 1)
    ...
    try:
        ...
        # AMBIL DATA & PAKSA HEADER JADI BESAR
        def get_df_fix(ws_name):
            df = pd.DataFrame(sh.worksheet(ws_name).get_all_records())
            df.columns = [str(c).strip().upper() for c in df.columns]
            return df

        df_staff = get_df_fix("Staff")
        df_absen = get_df_fix("Absensi")
        df_kas = get_df_fix("Arus_Kas")
        ...
        if len(raw_t) > 1:
            ...
            if len(df_tugas.columns) >= 5: df_tugas.columns.values[4] = "STATUS"
        else: df_tugas = pd.DataFrame()

        # FILTER TANGGAL ANTI-ERROR
        def filter_aman(df, col, bln, thn):
            if df.empty or col.upper() not in df.columns: return pd.DataFrame()
            df['TGL_OBJ'] = pd.to_datetime(df[col.upper()], dayfirst=True, errors='coerce')
            return df[df['TGL_OBJ'].apply(lambda x: x.month == bln and x.year == thn if pd.notnull(x) else False)].copy()

        df_t_bln = filter_aman(df_tugas, 'DEADLINE', bulan_dipilih, tahun_dipilih)
        df_a_f = filter_aman(df_absen, 'TANGGAL', bulan_dipilih, tahun_dipilih)
        df_k_f = filter_aman(df_kas, 'TANGGAL', bulan_dipilih, tahun_dipilih)
        ...
        if not df_t_bln.empty:
            for _, t in df_t_bln.sort_values('TGL_OBJ').iterrows():
                st.write(f"🟢 **{t['TGL_OBJ'].strftime('%d %b')}** - {t.get('INSTRUKSI')} ({t.get('STAF')})")
        else: st.caption("Tidak ada jadwal terbaca di bulan ini.")
        ...
        rekap_f = df_t_bln[df_t_bln['STATUS'].astype(str).str.upper() == "FINISH"]['STAF'].str.upper().value_counts().to_dict() if not df_t_bln.empty else {}
        
        pay = 0
        for _, s in df_staff.iterrows():
            n_up = str(s['NAMA']).upper()
            pay += (int(s['GAJI_POKOK']) + int(s['TUNJANGAN']) + (rekap_a.get(n_up, 0)*50000) + (rekap_f.get(n_up, 0)*10000))
        ...
        # 4. SLIP GAJI (RINCIAN FULL)
        with st.expander("💰 RINCIAN GAJI & SLIP (FULL)", expanded=True):
            for _, s in df_staff.iterrows():
                n_up = str(s['NAMA']).upper()
                ha, vi = rekap_a.get(n_up, 0), rekap_f.get(n_up, 0)
                tg = int(s['GAJI_POKOK']) + int(s['TUNJANGAN']) + (ha*50000) + (vi*10000)
                with st.container(border=True):
                    c1, c2, c3 = st.columns([2, 1, 1])
                    c1.write(f"👤 **{s['NAMA']}**"); c1.caption(f"💼 {s['JABATAN']}")
                    c2.write(f"📅 {ha} Hadir"); c3.write(f"🎬 {vi} Video")
                    if st.button(f"🧾 LIHAT SLIP {n_up}"):
                        slip = f"""<div style="background:white; color:black; padding:20px; border:4px solid #1d976c; border-radius:10px; width:300px; margin:auto;">
                        <center><img src="https://raw.githubusercontent.com/pintarkantor-prog/pintarmedia/main/PINTAR.png" width="100"><br><b>SLIP GAJI DETAIL</b><hr></center>
                        ...
                        <tr><td>Bonus Hadir ({ha}x)</td><td align="right">Rp {ha*50000:,}</td></tr>
                        <tr><td>Bonus Video ({vi}x)</td><td align="right">Rp {vi*10000:,}</td></tr>
                        <tr style="font-weight:bold; color:#1d976c;"><td>TOTAL</td><td align="right">Rp {tg:,}</td></tr></table></div>"""
                        st.components.v1.html(slip, height=450)

    except Exception as e: st.error(f"⚠️ Kendala: {e}")
